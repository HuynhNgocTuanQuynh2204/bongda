...
<?php
$sql = "SELECT * FROM loaitin WHERE IDtin = '$_GET[loaitin]'";
    ...
    <a href="index.php?quanly=loaitin" Class="btn btn-success" style="margin-left: 500px;">Loại tin</a>
        <div class="row d-flex justify-content-right align-items-right">
            <div class="col-5">
                ...
                <form action="index.php?quanly=xulythemloaitin&loaitin=<?php echo $row['IDtin'] ?>" method="POST" enctype="multipart/form-data">
                    <div class="mb-3">
                        <label class="form-label">Tên loại tin</label>
                        <input name="tenloaitin" type="text" class="form-control"  value="<?php echo $row['TenLoaiTin'] ?>">
                    </div>
                    <button type="submit" class="btn btn-primary" name="sualoaitin">Sửa </button>
                </form>
            </div>
